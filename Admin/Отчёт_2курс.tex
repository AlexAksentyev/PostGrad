...
\begin{document}
	...
	\begin{figure}[h]
		...
		\begin{minipage}{.5\textwidth}
			\centering
			\includegraphics[scale=.5]{img/ACF1_dens.eps}
			\caption{Распределение автокорреляции ошибки первого порядка.\label{fig:DW2}}
		\end{minipage}~~
		\begin{minipage}{.5\textwidth}
			\centering
			\includegraphics[scale=.5]{img/FStats_BP_dens.eps}
			\caption{Точка изменения величины наклона касательной, полученная движущимся тестом Чоу. Распределение циклов без мишени в верхней панели.\label{FStat_BP_dens}}
		\end{minipage}
	\end{figure}
